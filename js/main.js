...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      ...
      fetch(contactForm.action || 'https://formspree.io/f/xvzeyqen', {
        method: 'POST',
        body: formData,
...
function renderProjects() {
  ...
  const filterEl = document.getElementById('project-filters');
  ...
  if (filterEl) {
    filterEl.innerHTML = categories.map((cat, i) =>
      `<button class="cert-filter-btn ${i === 0 ? 'active' : ''}" data-cat="${cat}">${cat}</button>`
    ).join('');
    ...
    filterEl.querySelectorAll('.cert-filter-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const filtered = selected === 'Semua' ? projects : projects.filter(p => p.category === selected);
        renderProjectGrid(filtered, projectsContainer);
      });
    });
  }

  renderProjectGrid(projects, projectsContainer);
}
...
function initTypewriter() {
  ...
  const rolesID = ["Software Developer", "IT Support Specialist", "Warehouse & Logistics Staff"];
  const rolesEN = ["Software Developer", "IT Support Specialist", "Warehouse & Logistics Staff"];
  const roles = currentLang === 'en' ? rolesEN : rolesID;

  if (typewriterTimeout) clearTimeout(typewriterTimeout);

  function type() {
    const currentRole = roles[typewriterIdx % roles.length];

    if (typewriterIsDeleting) {
      el.textContent = currentRole.substring(0, typewriterCharIdx - 1);
      typewriterCharIdx--;
    } else {
      el.textContent = currentRole.substring(0, typewriterCharIdx + 1);
      typewriterCharIdx++;
    ...
    let typeSpeed = typewriterIsDeleting ? 50 : 100;

    if (!typewriterIsDeleting && typewriterCharIdx === currentRole.length) {
      typeSpeed = 2000;
...
function initStatCounters() {
  ...
  // Labels translation based on language
  const certLabel = document.getElementById('stat-label-cert');
  const projLabel = document.getElementById('stat-label-project');
  const expLabel = document.getElementById('stat-label-exp');

  if (currentLang === 'en') {
    if (certLabel) certLabel.textContent = 'Certificates & Training';
    if (projLabel) projLabel.textContent = 'Web & App Projects';
    if (expLabel) expLabel.textContent = 'Years Experience';
  } else {
    if (certLabel) certLabel.textContent = 'Sertifikat & Pelatihan';
    if (projLabel) projLabel.textContent = 'Project Web & App';
    if (expLabel) expLabel.textContent = 'Tahun Pengalaman';
  }
